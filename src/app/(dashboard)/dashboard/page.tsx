    ...
    return (
        <div>
            <header style={{ marginBottom: '2rem' }}>
                <h1 style={{ fontSize: 'clamp(1.5rem, 5vw, 2.5rem)', fontWeight: 700, letterSpacing: '-0.02em', marginBottom: '0.5rem' }}>Hallo, {user.name} 👋</h1>
                <p style={{ color: 'var(--foreground-muted)', fontSize: 'clamp(0.85rem, 3vw, 1rem)' }}>Willkommen zurück in deinem Lern-Cockpit.</p>
            </header>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px', marginBottom: '32px' }}>
                ...
                <Card title="Aktionen">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        ...
                        <a href="/collaboration/threads" style={{ textDecoration: 'none' }}>
                            <button style={{
                                width: '100%',
                                padding: '24px',
                                background: 'var(--surface-highlight)',
                    ...
                    {nextLecture ? (
                        <div style={{ padding: '12px 0' }}>
                            <div style={{ fontSize: '0.9rem', color: 'var(--accent)', marginBottom: '8px', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                {new Date(nextLecture.startTime).toLocaleDateString()} {new Date(nextLecture.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                            </div>
